    ...
    def step(self, action, raw_json=None):
        ...
        if cmd == "ls":
            ...
            if content:
                ...
                self.stderr = f"cat: {target}: {err}"

        elif cmd == "env":
            if self.current_phase < 2:
                self.stderr = "SYSTEM ERROR: Env monitoring offline."
            else:
                self.stdout = "USER=agent\nSHELL=/bin/bash\nROOT_PIN=FLAG{root_environment_secured}\nPATH=/usr/bin\nSESSION=active"

        elif cmd == "decode":
            if self.current_phase < 1:
                self.stderr = "SYSTEM ERROR: Utility offline."
            else:
                try:
                    self.stdout = base64.b64decode(target).decode("utf-8")
                except:
                    self.stderr = "decode: Invalid Base64 data."
